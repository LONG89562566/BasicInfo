        ...
        if (u) {
            switch (i) {
                case"file":
                    ...
                    break;
                case"video":
                    if (!RegExp("\\w\\.(" + (p || "avi|mp4|wma|rmvb|rm|flash|3gp|flv") + ")$", "i").test(escape(u))){
                        return t.msg("不支持该视频格式", r), e.value = "";
                    }
                    break;
                case"audio":
                    if (!RegExp("\\w\\.(" + (p || "mp3|wav|mid") + ")$", "i").test(escape(u))){
                        return t.msg("不支持该音频格式", r), e.value = "";
                    }
                    break;
                default:
                    if (!RegExp("\\w\\.(" + (p || "jpg|png|gif|bmp|jpeg") + ")$", "i").test(escape(u))){
