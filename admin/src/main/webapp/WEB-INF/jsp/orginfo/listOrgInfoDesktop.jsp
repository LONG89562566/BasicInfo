...
<html>
	...
	<body class="hold-transition skin-blue sidebar-mini">
	  ...
	  <div class="content-wrapper">
	    ...
	    <!-- Main content -->
	    <shiro:hasPermission name="orgInfo:query">
		    <section class="content">
		      <div class="row">
		        <div class="col-xs-12">
		          <div class="box">
		            <div class="box-header">
		              <h3 class="box-title">组织机构信息列表 :${sessionScope.login_session_admin.userName}</h3>
		            </div>
			        <div class="box box-info">
			           <!-- form start -->
			           <form  id="form_submit" class="form-horizontal" action="/admin/orgInfo/list" method="post">
			             ...
			             <table id="example1" class="table table-bordered table-striped">
			               <thead>
				              <tr>
			                    ...
			                    <th field="seq"  >排序号</th>
			                    <th field="level"  >层级</th>
			                    <th field="isEnd"  >是否末级</th>
			                    <th field="parentId"  >父编号</th>
			                    <th field="orgName"  >机构名称</th>
			                    <th field="otName"  >机构别名</th>
			                    <th field="orgCode"  >机构编码</th>
			               ...
			               <c:forEach items="${paginator.object}" var="r" varStatus="st"> 
						        ...
						        <td>
						         <div class="site-demo-button" >
								   <button id="updateOrgInfo" data-method="setAddOrEdit" value="${r.orgId}" class="layui-btn layui-btn-normal layui-btn-small"><i class="layui-icon"></i><span>&nbsp;&nbsp;修改</span></button>
								 </div>
						       </td>
				             </tr>
						   </c:forEach>
	...
	<script type="text/javascript">
		...
		//查询数据Url
		var pageQueryUrl = "<%=request.getContextPath()%>/admin/orgInfo/pageQuery";
		//查询条件表单Id
	...
	<script>
			...
			var active = {
				...
				offset: function(othis){
					...
					if(status.indexOf("禁用")!=-1){
						//禁用的url
						requestUrl="<%=request.getContextPath()%>/admin/orgInfo/disabled";
						text = "确定要禁用此条数据吗？";
					}else{
						//启用的url
						requestUrl="<%=request.getContextPath()%>/admin/orgInfo/enabled";
						text = "确定要启用此条数据吗？";
					}
					var id = othis.val();
		...
		//新增、编辑打开
		var setAddOrEdit = function(orgId){
		     //多窗口模式，层叠置顶
		     layer.open({
		         type: 2, 
		         title: '新增/修改 组织机构信息',
		         ...
		         content: '<%=request.getContextPath()%>/admin/orgInfo/addOrEdit?orgId='+orgId,
		         zIndex: layer.zIndex, //重点1
		         success: function(layero, index){
		        	 //layer.setAddOrEdit(layero);
		        	 var body = layer.getChildFrame('body', index);
		             var iframeWin = window[layero.find('iframe')[0]['name']]; 
		             body.find('input[name="orgId"]').val(orgId);
		             //弹窗表单的取消操作时关闭弹窗
