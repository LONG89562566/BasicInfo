...
<html>
			           ...
			           <div class="box-body">
			             ...
			             <table id="example1" class="table table-bordered table-striped">
			               ...
			               <c:forEach items="${paginator.object}" var="r" varStatus="st"> 
						        ...
						        <td>
						         <div class="site-demo-button" >
								   <button id="updateRepairInfo" data-method="setAddOrEdit" value="${r.repairId}" class="layui-btn layui-btn-normal layui-btn-small"><i class="layui-icon"></i><span>&nbsp;&nbsp;修改</span></button>
								 </div>
						       </td>
				             </tr>
						   </c:forEach>
	...
	<script type="text/javascript">
		...
		//查询数据Url
		var pageQueryUrl = "<%=request.getContextPath()%>/admin/repairInfo/pageQuery";
		//查询条件表单Id
	...
	<script>
			...
			var active = {
				...
				offset: function(othis){
					...
					if(status.indexOf("禁用")!=-1){
						//禁用的url
						requestUrl="<%=request.getContextPath()%>/admin/repairInfo/disabled";
						text = "确定要禁用此条数据吗？";
					}else{
						//启用的url
						requestUrl="<%=request.getContextPath()%>/admin/repairInfo/enabled";
						text = "确定要启用此条数据吗？";
					}
					var id = othis.val();
		...
		var setAddOrEdit = function(repairId){
		     ...
		     layer.open({
		         ...
		         content: '<%=request.getContextPath()%>/admin/repairInfo/addOrEdit?repairId='+repairId,
