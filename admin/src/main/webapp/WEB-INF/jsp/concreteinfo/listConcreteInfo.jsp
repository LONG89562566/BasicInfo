...
<html>
	...
	<body class="hold-transition skin-blue sidebar-mini">
	  ...
	  <div class="content-wrapper">
	    ...
	    <!-- Main content -->
	    <shiro:hasPermission name="concreteInfo:query">
		        ...
		        <div class="col-xs-12">
		          <div class="box">
		            <div class="box-header">
		              <h3 class="box-title">混泥土信息列表 :${sessionScope.login_session_admin.userName}</h3>
		            </div>
			           ...
			           <form  id="form_submit" class="form-horizontal"  method="post">
			              ...
			              <div class="box-body">
			                 <div class="form-group">
								 <div class="form-group">
									 <label for="name" class="col-sm-2 control-label">混凝土别名：</label>
									 <div class="col-xs-2">
										 <input type="text" class="form-control" id="name" name="name" value="${concreteInfo.name}" placeholder="请输入混凝土别名">
									 </div>
								 </div>
			                 </div>
			           ...
			           <div class="box-body">
			             ...
			             <table id="example1" class="table table-bordered table-striped">
			               ...
			               <c:forEach items="${paginator.object}" var="r" varStatus="st"> 
						        ...
						        <td>
						         <div class="site-demo-button" >
								   <button id="updateConcreteInfo" data-method="setAddOrEdit" projectId="${r.projectId}" value="${r.concreteId}" class="layui-btn layui-btn-normal layui-btn-small"><i class="layui-icon"></i><span>&nbsp;&nbsp;修改</span></button>
								   <button id="deletes" data-method="deletes" value="${r.concreteId}" class="layui-btn layui-btn-danger layui-btn-small"><i class="layui-icon"></i><span>&nbsp;&nbsp;删除</span></button>
								 </div>
						       </td>
				             </tr>
						   </c:forEach>
	...
	<script type="text/javascript">
		...
		//查询数据Url
		var pageQueryUrl = "<%=request.getContextPath()%>/admin/concreteInfo/pageQuery";
		//查询条件表单Id
	...
	<script>
			...
			var active = {
				...
				offset: function(othis){
					...
					if(status.indexOf("禁用")!=-1){
						//禁用的url
						requestUrl="<%=request.getContextPath()%>/admin/concreteInfo/disabled";
						text = "确定要禁用此条数据吗？";
					}else{
						//启用的url
						requestUrl="<%=request.getContextPath()%>/admin/concreteInfo/enabled";
						text = "确定要启用此条数据吗？";
					}
					var id = othis.val();
		...
		//新增、编辑打开
		var setAddOrEdit = function(concreteId,projectId){
            if(!projectId){
                layer.msg('请先选择一个梁场！');
                return;
            }
		     ...
		     layer.open({
		         ...
		         content: '<%=request.getContextPath()%>/admin/concreteInfo/addOrEdit?concreteId='+concreteId+"&projectId="+projectId,
		         ...
		         success: function(layero, index){
		             ...
		             body.find('input[name="concreteId"]').val(concreteId);
		             //弹窗表单的取消操作时关闭弹窗
        ...
        var deletes = function (id) {
            //启用的url
            requestUrl="<%=request.getContextPath()%>/admin/concreteInfo/delete";
            text = "确定要删除条数据吗？";
