...
<html>
	...
	<body class="hold-transition skin-blue sidebar-mini">
	  ...
	  <div class="content-wrapper">
	    ...
	    <!-- Main content -->
	    <shiro:hasPermission name="releaseInfoLog:query">
		    <section class="content">
		      <div class="row">
		        <div class="col-xs-12">
		          <div class="box">
		            <div class="box-header">
		              <h3 class="box-title">信息发布日志列表 :${sessionScope.login_session_admin.userName}</h3>
		            </div>
			        <div class="box box-info">
			           <!-- form start -->
			           <form  id="form_submit" class="form-horizontal" action="/admin/releaseInfoLog/list" method="post">
			             ...
			             <table id="example1" class="table table-bordered table-striped">
			               <thead>
				              <tr>
			                    ...
			                    <th field="createTime"  type='date'>注册时间</th>
			                    <th field="releaseInfoId"  >信息发布id</th>
			                    <th field="accepter"  >接收人id</th>
			                    <th field="isRead"  >回报率</th>
			                    <th field="readTime"  >阅读时间</th>
			               ...
			               <c:forEach items="${paginator.object}" var="r" varStatus="st"> 
						        ...
						        <td>
						         <div class="site-demo-button" >
								   <button id="updateReleaseInfoLog" data-method="setAddOrEdit" value="${r.releaseInfoLogId}" class="layui-btn layui-btn-normal layui-btn-small"><i class="layui-icon"></i><span>&nbsp;&nbsp;修改</span></button>
								 </div>
						       </td>
				             </tr>
						   </c:forEach>
	...
	<script type="text/javascript">
		...
		//查询数据Url
		var pageQueryUrl = "<%=request.getContextPath()%>/admin/releaseInfoLog/pageQuery";
		//查询条件表单Id
	    ...
	    //主键
	    var primarykey = "releaseInfoLogId";
	    //分页显示标签id
	...
	<script>
			...
			var active = {
				...
				offset: function(othis){
					...
					if(status.indexOf("禁用")!=-1){
						//禁用的url
						requestUrl="<%=request.getContextPath()%>/admin/releaseInfoLog/disabled";
						text = "确定要禁用此条数据吗？";
					}else{
						//启用的url
						requestUrl="<%=request.getContextPath()%>/admin/releaseInfoLog/enabled";
						text = "确定要启用此条数据吗？";
					}
					var id = othis.val();
		...
		//新增、编辑打开
		var setAddOrEdit = function(releaseInfoLogId){
		     //多窗口模式，层叠置顶
		     layer.open({
		         type: 2, 
		         title: '新增/修改 信息发布日志',
		         ...
		         content: '<%=request.getContextPath()%>/admin/releaseInfoLog/addOrEdit?releaseInfoLogId='+releaseInfoLogId,
		         zIndex: layer.zIndex, //重点1
		         success: function(layero, index){
		        	 //layer.setAddOrEdit(layero);
		        	 var body = layer.getChildFrame('body', index);
		             var iframeWin = window[layero.find('iframe')[0]['name']]; 
		             body.find('input[name="releaseInfoLogId"]').val(releaseInfoLogId);
		             //弹窗表单的取消操作时关闭弹窗
