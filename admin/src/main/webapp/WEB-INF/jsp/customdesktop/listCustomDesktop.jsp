...
<html>
		        ...
		        <div class="col-xs-12">
		          <div class="box">
		            <div class="box-header">
		              <h3 class="box-title">自定义桌面列表 :${sessionScope.login_session_admin.userName}</h3>
		            </div>
			        <div class="box box-info">
			           <!-- form start -->
			           <form  id="form_submit" class="form-horizontal" action="/admin/customDesktop/list" method="post">
			           ...
			           <div class="box-body">
			             ...
			             <table id="example1" class="table table-bordered table-striped">
			               ...
			               <c:forEach items="${paginator.object}" var="r" varStatus="st"> 
						        ...
						        <td>
						         <div class="site-demo-button" >
								   <button id="updateCustomDesktop" data-method="setAddOrEdit" value="${r.desktopId}" class="layui-btn layui-btn-normal layui-btn-small"><i class="layui-icon"></i><span>&nbsp;&nbsp;修改</span></button>
								 </div>
						       </td>
				             </tr>
						   </c:forEach>
	...
	<script type="text/javascript">
		...
		//查询数据Url
		var pageQueryUrl = "<%=request.getContextPath()%>/admin/customDesktop/pageQuery";
		//查询条件表单Id
	...
	<script>
			...
			var active = {
				...
				offset: function(othis){
					...
					if(status.indexOf("禁用")!=-1){
						//禁用的url
						requestUrl="<%=request.getContextPath()%>/admin/customDesktop/disabled";
						text = "确定要禁用此条数据吗？";
					}else{
						//启用的url
						requestUrl="<%=request.getContextPath()%>/admin/customDesktop/enabled";
						text = "确定要启用此条数据吗？";
					}
					var id = othis.val();
		...
		//新增、编辑打开
		var setAddOrEdit = function(desktopId){
		     //多窗口模式，层叠置顶
		     layer.open({
		         type: 2, 
		         title: '新增/修改 自定义桌面',
		         ...
		         content: '<%=request.getContextPath()%>/admin/customDesktop/addOrEdit?desktopId='+desktopId,
		         zIndex: layer.zIndex, //重点1
		         success: function(layero, index){
		        	 //layer.setAddOrEdit(layero);
		        	 var body = layer.getChildFrame('body', index);
		             var iframeWin = window[layero.find('iframe')[0]['name']]; 
		             body.find('input[name="desktopId"]').val(desktopId);
		             //弹窗表单的取消操作时关闭弹窗
