...
<html>
	...
	<body class="hold-transition skin-blue sidebar-mini">
	  ...
	  <div class="content-wrapper">
	    ...
	    <!-- Main content -->
	    <shiro:hasPermission name="proportionsDetailetail:query">
		    <section class="content">
		      <div class="row">
		        <div class="col-xs-12">
		          <div class="box">
		            <div class="box-header">
		              <h3 class="box-title">配合比详情列表 :${sessionScope.login_session_admin.userName}</h3>
		            </div>
			        <div class="box box-info">
			           <!-- form start -->
			           <form  id="form_submit" class="form-horizontal" action="/admin/proportionsDetailetail/list" method="post">
	...
	<script type="text/javascript">
		...
		//查询数据Url
		var pageQueryUrl = "<%=request.getContextPath()%>/admin/proportionsDetailetail/pageQuery";
		//查询条件表单Id
	...
	<script>
			...
			var active = {
				...
				offset: function(othis){
					...
					if(status.indexOf("禁用")!=-1){
						//禁用的url
						requestUrl="<%=request.getContextPath()%>/admin/proportionsDetailetail/disabled";
						text = "确定要禁用此条数据吗？";
					}else{
						//启用的url
						requestUrl="<%=request.getContextPath()%>/admin/proportionsDetailetail/enabled";
						text = "确定要启用此条数据吗？";
					}
					var id = othis.val();
		...
		//新增、编辑打开
		var setAddOrEdit = function(detailId){
		     //多窗口模式，层叠置顶
		     layer.open({
		         type: 2, 
		         title: '新增/修改 配合比详情',
		         ...
		         success: function(layero, index){
		             ...
		             body.find('input[name="detailId"]').val(detailId);
		             //弹窗表单的取消操作时关闭弹窗
