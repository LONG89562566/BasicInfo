...
<html>
		        ...
		        <div class="col-xs-12">
		          <div class="box">
		            <div class="box-header">
		              <h3 class="box-title">材料出入库列表 :${sessionScope.login_session_admin.userName}</h3>
		            </div>
			        <div class="box box-info">
			           <!-- form start -->
			           <form  id="form_submit" class="form-horizontal" action="/admin/materialDetail/outList" method="post">
			              ...
			              <div class="box-body">
			                 <div class="form-group">
								 <div class="form-group">
									 <label for="outName" class="col-sm-2 control-label">物品出库批次名称：</label>
									 <div class="col-xs-2">
										 <input type="text" class="form-control" id="outName" name="outName" value="" placeholder="请输入物品物品出库批次名称">
									 </div>
								 </div>
			                 </div>
			             ...
			             <table id="example1" class="table table-bordered table-striped">
			               <thead>
				              <tr>
			                    ...
			                    <th field="outName"  >出库批次名称</th>
			                    <th field="testState"  method="testState" >检验状态</th>
			               ...
			               <c:forEach items="${paginator.object}" var="r" varStatus="st"> 
					            ...
					            <td><c:if test="${r.materialType == 0 }">出库</c:if>
									<c:if test="${r.materialType == 1 }">入库</c:if></td>
					            <td>${r.outName}</td>
					            <td><c:if test="${r.testState == null }"><font color="red">未送检</font></c:if>
									<c:if test="${r.testState == 0 }"><font color="red">检验未通过</font></c:if>
									<c:if test="${r.testState == 1 }"><font color="green">检验通过</font></c:if>
									<c:if test="${r.testState == 2 }"><font color="red">检验中</font></c:if>
								</td>
						        <td>
						         <div class="site-demo-button" >
	...
	<script type="text/javascript">
		...
		//查询数据Url
		var pageQueryUrl = "<%=request.getContextPath()%>/admin/materialDetail/outPageQuery";
		//查询条件表单Id
	    ...
	    tableBtn = addBtn(tableBtn,"outAddOrEdit","检验","","","","","","layui-btn-normal");
	...
	<script type="text/javascript">
        ...
        var testState = function (val , obj) {
            var retVal = "";
            if (val != null) {
                retVal = "<font color=\"red\">未送检</font>";
            }
            if (val == 0) {
                retVal = "<font color=\"red\">检验未通过</font>";
            }
            if (val == 1) {
                retVal = "<font color=\"green\">检验通过</font>";
            }
            if (val == 2) {
                retVal = "<font color=\"red\">检验中</font>";
            }
	...
	<script>
		...
		layui.use('layer', function(){ //独立版的layer无需执行这一句
			...
			var active = {
                ...
                outAddOrEdit: function(data){
                    //获取materialDetailId
                    var id = data.val();
                    outAddOrEdit(id);
                },
                outMaterialDetail: function(data){
                    //获取materialDetailId
                    var id = data.val();
                    outMaterialDetail(id);
                },
			};
		...
		var setAddOrEdit = function(materialDetailId,projectId){
            if(!projectId){
		     ...
		     layer.open({
		         ...
		         content: '<%=request.getContextPath()%>/admin/materialDetail/outOrEdit?materialDetailId='+materialDetailId+'&projectId='+projectId,
        ...
        //检验打开
        var outAddOrEdit = function(outId){
            //多窗口模式，层叠置顶
            layer.open({
                type: 2,
                title: ' 检验 出库',
                ...
                shade: 0.5,
                anim: 3,//0-6的动画形式，-1不开启
                content: '<%=request.getContextPath()%>/admin/materialDetail/outTest?outId='+outId,
                zIndex: layer.zIndex, //重点1
                success: function(layero, index){
                    //layer.setAddOrEdit(layero);
                    var body = layer.getChildFrame('body', index);
        ...
        //出库
        var outMaterialDetail = function(outId){
            //多窗口模式，层叠置顶
            var text = "确定要出库吗？";
            layer.open({
                type: 1,
                ...
                yes: function(){
                    ...
                    $.ajax({
                        type: "POST",
                        url: '<%=request.getContextPath()%>/admin/materialDetail/outTestMaterial?outId='+outId,
                        dataType: "json",
                        cache:false,
